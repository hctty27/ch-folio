...
function optionalCommit(value)
{
    ...
    if(!GIT_COMMIT.test(commit))
        throw new TypeError('commit must be an exact 40-character hexadecimal Git SHA')
    return commit
}
...
export function parseNodeLoadTestOptions(argv = process.argv.slice(2), environment = process.env)
{
    ...
    const commit = optionalCommit(
        args.get('commit') ?? environment.AUTHORITATIVE_BENCHMARK_COMMIT,
    )
...
export function buildNodeBenchmarkWebSocketUrl(baseUrl, room)
{
...
export function evaluateNodeLoadTestGates(report)
{
    ...
    const queue = finiteOrInfinity(report?.server?.gauges?.maxQueueDepth)
    ...
    if(queue > 3) failure(failures, 'server.gauges.maxQueueDepth', queue, 3)
...
export async function runNodeLoadTest(options)
{
    ...
    try
    {
        ...
        if(serverSummary.mode !== 'node')
            throw new Error(`expected Node benchmark summary, received ${serverSummary.mode ?? 'unknown'}`)
        ...
        const report = {
            ...
            server: {
                ...serverMetrics,
                observedPeakMemoryBytes: Number(serverSummary.observedPeakMemoryBytes ?? Number.NaN),
            ...
            roomRestarts: Number(serverSummary.roomRestarts ?? Number.POSITIVE_INFINITY),
            serverSummaryMetadata: {
                ...
                rapierInternalTimingAvailable: serverSummary.rapierInternalTimingAvailable,
            },
        }
