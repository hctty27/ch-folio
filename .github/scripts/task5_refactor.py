...
physics_path = Path('sources/Game/Physics/PhysicsVehicle.js')
physics = physics_path.read_text()

if "from '@ch-folio/authoritative-physics'" not in physics:
    physics = physics.replace(
        "import { Events } from '../Events.js'\n",
        "import { Events } from '../Events.js'\nimport {\n    VEHICLE_CONFIG,\n    applyVehicleInput,\n    createQuantizedInputFromPlayer,\n} from '@ch-folio/authoritative-physics'\n",
    )

    old_tuning = """        this.steeringAmplitude = 0.5
...
"""
    new_tuning = """        this.steeringAmplitude = VEHICLE_CONFIG.steeringAmplitude
...
"""
    if old_tuning not in physics:
        raise SystemExit('constructor tuning block not found')
    physics = physics.replace(old_tuning, new_tuning)
...
"""
    new_colliders = """            colliders: VEHICLE_CONFIG.chassis.colliders.map((collider) => ({
                ...collider,
...
"""
    if old_colliders not in physics:
        raise SystemExit('chassis collider block not found')
    physics = physics.replace(old_colliders, new_colliders)
    ...
    old_settings = """        this.wheels.settings = {
...
"""
    new_settings = """        this.wheels.settings = {
            offset: {
...
"""
    if old_settings not in physics:
        raise SystemExit('wheel settings block not found')
    physics = physics.replace(old_settings, new_settings)

    old_positions = """            const wheelsPositions = [
...
"""
    new_positions = """            const wheelsPositions = VEHICLE_CONFIG.wheels.positions.map(([ x, y, z ]) => new THREE.Vector3(
                Math.sign(x) * this.wheels.settings.offset.x,
                y + this.wheels.settings.offset.y,
                Math.sign(z) * this.wheels.settings.offset.z,
            ))
"""
    if old_positions not in physics:
        raise SystemExit('wheel positions block not found')
    physics = physics.replace(old_positions, new_positions)

    new_prephysics = """    updatePrePhysics()
    {
    ...
    if count != 1:
        raise SystemExit('updatePrePhysics block not found')

    physics_path.write_text(physics)

player_path = Path('sources/Game/Player.js')
player = player_path.read_text()
if 'this.honking = 0' not in player:
...
index_path = Path('packages/authoritative-physics/src/index.js')
index = index_path.read_text()
if "from './vehicleConfig.js'" not in index:
    index += """
